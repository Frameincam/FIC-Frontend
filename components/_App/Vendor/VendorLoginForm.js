...
const VendorLoginForm = ({ open }) => {
	...
	const vendorLoginSubmit = async (e) => {
		const d = {
			mobile: e.mobile,
			password: e.password,
		};
		...
		try {
			const { data } = await axios.post(
				`${process.env.DOMAIN_NAME}/api/account/vendor/login`,
				d
			);
			console.log(data);
	...
	const googleVendorLoginSuccess = async (res) => {
		...
		try {
			const google = await axios.post(
				`${process.env.DOMAIN_NAME}/api/account/vendor/google-login`,
				da
			);
	...
	const responseFacebook = async (response) => {
		...
		try {
			const faceBookLoginApi = await axios.post(
				`${process.env.DOMAIN_NAME}/api/account/vendor/facebook-login`,
				data
			);
			console.log(faceBookLoginApi);
			const token = faceBookLoginApi.data.token;
			const user = faceBookLoginApi.data.user;
		...
		<>
			...
			<div className="tab-pane fade show active" id="login">
				<div className="miran-login">
					...
					<form onSubmit={handleSubmit(vendorLoginSubmit)}>
